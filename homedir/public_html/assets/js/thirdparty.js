var i_body=0;
var coverage_percant = 0;
var coverage_price = 0;
var offs_percant = 0;
var offs_price = 0;
$(document).ready(function() {
    VehicleType();
	OffsThirdparty();
    $('.aghsatli').css('display', 'none');
    $("#datepicker").persianDatepicker({
        autoClose: true,
        onSelect: function(unix) {
            tdate = new persianDate(unix)
            var date = new persianDate()
            var c = date.diff(tdate, 'days')
            if (date > tdate) {
                tg = 0;
                diffDays = c
            } else {
                diffDays = 0
            }
            change_price()
        }
    });
    if (machine) {
        GetPrice();
    } else {
		$('#CompareDiv').html('<div class="ant-row-flex ant-row-flex-middle itemList hasgift" style="background:#fff1f0;border:1px solid #ffa39e">' +
                    '<div class="ant-col-xs-24 ant-col-sm-24 ant-col-md-24 ant-col-lg-24">' +
                    '<div class="ant-progress ant-progress-line ant-progress-status-active ant-progress-small" style="height: 40px; line-height: 50px; text-align:center;">' +
                    'توجه! برای نمایش بسته های پیشنهادی بیمه شخص ثالث ، اطلاعات مورد نیاز ( مشخص شده) را وارد نمایید.</div>' +
                    '</div></div>')
        $('.block_description').css('display', 'block');
    }
})

function morecom(id) {
    var name = $('#morecome' + id).attr('name');
    $('.divcom').css('display', 'none')
    if (name == 'no') {
        $('#com' + id).css('display', 'block')
        $('#morecome' + id).attr('name', 'yes')
    } else {
        $('#com' + id).css('display', 'none')
        $('#morecome' + id).attr('name', 'no')
    }
}

function goToTwoLevel(avalprice, offprice, company) {
    var fullurl = window.location.href;
    var type = $("#VehicleType option:selected").text();
    var mach = $("#vehicleBrand option:selected").text();
    var mod = $("#vehicleModel option:selected").text();
    var old_company = $("#old_company option:selected").text();
    var credit = $("#credit option:selected").text();
    var has_damage = $(".has_damage option:selected").text();
    var s = $("#sal option:selected").text();
    var o = $("#thirdDiscount option:selected").text();
    var datepicker = $("#datepicker").val();
    var DriverDiscount = $("#DriverDiscount option:selected").text();
	var poshesh1 = $("#poshesh option:selected").text();
	var PropertyDamage = $("#PropertyDamage option:selected").text();
	var NumberDamages = $("#NumberDamages option:selected").text();
	var LossLife = $("#LossLife option:selected").text();
	var broker = $("#broker").val();
    $('.tsalesprice').html($.number(offprice) + ' تومان')
    $('.namecompany').html(company)
    $('.machine_name').html(mach)
    $('.modeli_name').html(mod)
    $('.sal_name').html(s)
    $('.off_name').html(o)
    $('.VehicleType').html(type)
    $('.datepicker').html(datepicker)
    $('.DriverDiscount').html(DriverDiscount)
    
    $('.poshesh').html(poshesh1)

    var carusage = type;
    if (username) {
        $.ajax({
            type: "GET",
            url: "/webservice/insurance/l1",
            data: "sal=" + s + "&machini=" + mach + "&modeli=" + mod + "&takhfif=" + o + "&type=" + carusage + "&CashIinstallments=نقد&url=" + fullurl + '&salesPrice=' + offprice + "&company=" + company + '&DriverDiscount=' + DriverDiscount + "&datepicker=" + datepicker + "&poshesh=" + poshesh1+"&old_company="+old_company+"&credit="+credit+"&has_damage="+has_damage+"&PropertyDamage="+PropertyDamage+"&NumberDamages="+NumberDamages+"&LossLife="+LossLife+"&broker="+broker,
            success: function() {}
        })
        $('.divmain').css('display', 'none')
        $('.divpic').css('display', 'block')
    } else {
        $('#divlogin').css('display', 'flex')
    }
}

function goToTwoAghsat(Installments_title_third, number_installments_third, avalprice,offprice,company) {
    var fullurl = window.location.href;
    var type = $("#VehicleType option:selected").text();
    var mach = $("#vehicleBrand option:selected").text();
    var mod = $("#vehicleModel option:selected").text();
    var old_company = $("#old_company option:selected").text();
    var credit = $("#credit option:selected").text();
    var has_damage = $(".has_damage option:selected").text();
    var s = $("#sal option:selected").text();
    var o = $("#thirdDiscount option:selected").text();
    var datepicker = $("#datepicker").val();
    var DriverDiscount = $("#DriverDiscount option:selected").text();
	var poshesh1 = $("#poshesh option:selected").text();
	var PropertyDamage = $("#PropertyDamage option:selected").text();
	var NumberDamages = $("#NumberDamages option:selected").text();
	var LossLife = $("#LossLife option:selected").text();
	var broker = $("#broker").val();
    $('.tsalesprice').html($.number(offprice) + ' تومان')
    $('.namecompany').html(company)
    $('.machine_name').html(mach)
    $('.modeli_name').html(mod)
    $('.sal_name').html(s)
    $('.off_name').html(o)
    $('.VehicleType').html(type)
    $('.datepicker').html(datepicker)
    $('.DriverDiscount').html(DriverDiscount)
    
    $('.poshesh').html(poshesh1)

    var carusage = type;
    if (username) {
        $.ajax({
            type: "GET",
            url: "/webservice/insurance/l1",
            data: "sal=" + s + "&machini=" + mach + "&modeli=" + mod + "&takhfif=" + o + "&type=" + carusage + "&CashIinstallments=اقساط&url=" + fullurl + '&salesPrice=' + offprice + "&company=" + company + '&DriverDiscount=' + DriverDiscount + "&datepicker=" + datepicker + "&poshesh=" + poshesh1+"&old_company="+old_company+"&credit="+credit+"&has_damage="+has_damage+"&PropertyDamage="+PropertyDamage+"&NumberDamages="+NumberDamages+"&LossLife="+LossLife+"&broker="+broker+"&Installments_title_third="+Installments_title_third+"&number_installments_third="+number_installments_third,
            success: function() {}
        })
        $('.divmain').css('display', 'none')
        $('.divpic').css('display', 'block')
    } else {
        $('#divlogin').css('display', 'flex')
    }
}

function block_pic() {
    $('#block_pic').css('display', 'none')
    $('#block_address').css('display', 'block')
}

function back_pic() {
    window.scrollTo(0, 0);
    $('.divpic').css('display', 'none')
    $('.divmain').css('display', 'block')
}

function block_address() {
    $('#block_address').css('display', 'none')
    $('#block_time').css('display', 'block')
}

function back_address() {
    window.scrollTo(0, 0);
    $('#block_address').css('display', 'none')
    $('#block_pic').css('display', 'block')
}

function back_payment() {
    $('#block_address').css('display', 'block')
    $('#block_payment').css('display', 'none')
}

function block_time() {
    $('.flname1').html($('#flname').val());
    $('.phone1').html($('#phone').val());
    $('.cellphone1').html($('#cellphone').val());
    $('.address1').html($('#city').val() + ' , ' + $('#address').val());
    var name = $('#flname').val();
    var phone = $('#phone').val();
    var cellphone = $('#cellphone').val();
    var address = $('#address').val();
    var city = $('#city').val();
    var f = 0;
    if (name && phone && cellphone && address && city) {
        f = 1;
    } else {
        f = 0;
        if (!name) {
            $('#flname').css('border-color', 'rgb(238, 111, 117)');
        } else {
            $('#flname').css('border-color', '#cacaca');
        }
        if (!phone) {
            $('#phone').css('border-color', 'rgb(238, 111, 117)');
        } else {
            $('#phone').css('border-color', '#cacaca');
        }
        if (!cellphone) {
            $('#cellphone').css('border-color', 'rgb(238, 111, 117)');
        } else {
            $('#cellphone').css('border-color', '#cacaca');
        }
        if (!address) {
            $('#address').css('border-color', 'rgb(238, 111, 117)');
        } else {
            $('#address').css('border-color', '#cacaca');
        }
        if (!city) {
            $('.divstate').css('border', '1px solid rgb(238, 111, 117)');
        } else {
            $('.divstate').css('border', '1px solid #d9d9d9');
        }
        window.scrollTo(0, 0);
    }
    if (f == '1') {
        var datai = $('#reciverForm').serialize();
        $.ajax({
            url: "/webservice/insurance/l2",
            type: "GET",
            data: datai,
            contentType: false,
            cache: false,
            processData: false,
            success: function(data) {
                $('#block_address').css('display', 'none');
                $('#block_payment').css('display', 'block')
                window.scrollTo(0, 0);
            }
        });
    } else {
        $('.banki').val('تایید و پرداخت');
    }

}

function back_time() {
    $('#block_time').css('display', 'none')
    $('#block_address').css('display', 'block')
}

function change_price() {
    GetPrice();
}

function GetPrice() {
    $('#CompareDiv').html('<div class="ant-row-flex ant-row-flex-middle itemList hasgift" style="height:250px;background-image:url(/assets/img/dots_3.gif);background-position:top center;background-repeat:no-repeat;background-size:20%;"></div>');
    $('.block_description').css('display', 'none');
    var model = $('#vehicleModel').val().split(',');
    var year = $('#sal').val();
    var offcode = $('#codeoffhidden').val();
    var cover = $('#poshesh').val();
    var takhfif = $('#thirdDiscount').val();
    var DriverDiscount = $('#DriverDiscount').val();
    var VehicleType = $('#VehicleType').val();
    var vehicleBrand = $('#vehicleBrand').val();
    var order = $('#order_company').val();
    var datepicker = $('#datepicker').val();
    var old_company = $('#old_company').val();
    var credit = $('#credit').val();
    var off_year = 0;
    var LossLife = $('#LossLife').val();
    var PropertyDamage = $('#PropertyDamage').val();
    var NumberDamages = $('#NumberDamages').val();
    /*if (LossLife) {
        LossLife = LossLife;
    }else{
	    LossLife=0;
	}*/
    if (year < 1383) {
        off_year = parseInt(2) * (parseInt(1383) - parseInt(year));
    }
    if (off_year > 20) {
        off_year = 20;
    }
    //LossLife=parseInt(LossLife) + parseInt(off_year);

    if (!order) {
        order = 'low';
    }
    var date1 = datepicker.replace("-", "/");
    var newdatepicker = date1.replace("-", "/");
    var url = "/thirdparty/" + VehicleType + "/" + vehicleBrand.replace(" ", "_") + "/" + model + "/" + newdatepicker;
    //document.title = response.pageTitle;
    //window.history.pushState("","",url);

    var b = 0,
        z = 0,
        dirkard = '',
        dirkard_price = 0,
        roz_dirkard = 0,
        price_payeh = 0,
        takhfifcode = 0,
        gg = 0;
    var t = '';
    $.getJSON('/api/price/thirdparty', {
        type: model[1],
        sal: year,
        off: takhfif,
        driver_discount: DriverDiscount,
        noe: VehicleType,
        poshesh: cover,
        orderi: order,
        date: datepicker,
        order_company: 'dana',
        oldcompany: old_company,
        property_damage: PropertyDamage,
        loss_life: LossLife,
        number_damages: NumberDamages,
        diff_Days: diffDays,
		offspercant : offs_percant,
		offsprice : offs_percant
    }, function(result) {
        $.each(result, function(i, field) {
            var offprice = field['tariff'].split(',').join('');
            var avalprice = field['aval'].split(',').join('');
            var codeoff = offcode.split('#');
            var darsadi = '';
            var price = 0;
            if (field['delay'].split(',').join('') > 0) {
                dirkard = '<div class="DiscountBox" style="right:0; bottom: 0;top:calc(100% - 30px)"><div class="giftdiscount" style="background-color:#bf0808"><span>دیرکرد : ' + field['delay'] + " تومان " + '</span></div></div>';
            }

            if (field['off'] > 0) {
                darsadi = '<div class="DiscountBox" style="right:0"><div class="giftdiscount"><span>' + field['off'] + '%</span></div></div>';
                var off_price = '<span style="text-decoration:line-through;float:right;color: #ec0909;font-weight:600;font-size:13px">' + $.number(parseInt(avalprice) / parseInt(credit)) + " تومان </span>";
            } else {
                var off_price = '';
            }

            if (field['off'] > 0) {
                if (codeoff[0] > 0) {
                    if (codeoff[1] == 'percent') {
                        takhfifcode = (parseInt(codeoff[0]) * parseInt(offprice)) / 100;
                    } else if (codeoff[1] == 'cash') {
                        takhfifcode = codeoff[0];
                    }
                    darsadi = '<div class="DiscountBox" style="right:20px"><div class="giftdiscount"><span>' + field['off'] + '% + ' + $.number(takhfifcode) + ' تومان تخفیف</span></div></div>';
                    var off_price = '<span style="text-decoration:line-through;float:right;color: #ec0909;font-weight:600;font-size:13px">' + $.number(((parseInt(avalprice) + parseInt(gg)) * parseInt(credit)) / 100) + " تومان </span>";
                } else {
                    var new_price = ((parseInt(avalprice) + parseInt(gg)) * parseInt(credit)) / 100
                    new_price = parseInt(new_price)
                    darsadi = '<div class="DiscountBox" style="right:20px"><div class="giftdiscount"><span>' + field['off'] + '% تخفیف</span></div></div>';
                    var off_price = '<span style="text-decoration:line-through;float:right;color: #ec0909;font-weight:600;font-size:13px">' + $.number(new_price) + " تومان </span>";
                }
            } else {
                var off_price = '';
            }
            /*if (takhfifcode > 0) {
                var newprice = parseInt(offprice) - parseInt(takhfifcode);
                var new_price = ((parseInt(newprice) + parseInt(gg)) * parseInt(credit)) / 100;
                new_price = parseInt(new_price)
                price = $.number() + " تومان ";
            } else {*/
                var new_price = ((parseInt(offprice) + parseInt(gg)) * parseInt(credit)) / 100;
                new_price = parseInt(new_price)
                price = $.number(new_price) + " تومان ";
            //}

            var offl = $.number(field['off']);
            var payeh = $.number(parseInt(offprice) + parseInt(field['off']));

            var btnAghsat = '';
            var btnCashIinstallments = '';
            var padd = 'padding-top:20px;';
            if (website_status == 'on') {
             if (field['cash_third'] != 0) {
                padd = 'padding:0;';
                btnCashIinstallments = '<button onClick="goToTwoLevel(&apos;' + avalprice + '&apos;,&apos;' + offprice + '&apos;,&apos;' + field['company_name'] + '&apos;)" data-toggle="modal" type="button" class="ant-btn btn-order-new ant-btn-lg" data-step="2" data-intro="برای سفارش بیمه نامه , از اینجا اقدام کنید"><i class="anticon anticon-shopping-cart"></i><span style="font-size:14px">سفارش نقدی</span></button>';
             }
             if (field['credit_third'] != 0) {
                btnAghsat = '<button onClick="goToTwoAghsat(&apos;'+field['Installments_title_third']+'&apos;,&apos;'+field['number_installments_third']+'&apos;,&apos;' + avalprice + '&apos;,&apos;' + offprice + '&apos;,&apos;' + field['company_name'] + '&apos;)" data-toggle="modal" type="button" class="ant-btn btn-order-new ant-btn-lg" style="margin-top:10px"><i class="anticon anticon-shopping-cart"></i><span style="font-size:14px">'+field['prepayment_third']+'</span></button>';
             }
            }
            var star = '<ul class="ant-rate ant-rate-disabled" tabindex="-1">'
            for (var i = 0; i < field['solvency_level']; i++) {
                star = star + '<li class="ant-rate-star ant-rate-star-full"><div class="ant-rate-star-first"><i class="anticon anticon-star"></i></div><div class="ant-rate-star-second"><i class="anticon anticon-star"></i></div></li>';
            }
            for (var j = field['solvency_level']; i < 5; i++) {
                star = star + '<li class="ant-rate-star ant-rate-star-zero"><div class="ant-rate-star-first"><i class="anticon anticon-star"></i></div><div class="ant-rate-star-second"><i class="anticon anticon-star"></i></div></li>';
            }
            star = star + '</ul>';
			var saiar='<i aria-label="icon: close" class="anticon anticon-close"></i>';
            if(field['saiar']==='دارد'){
			  saiar='<i aria-label="icon: close" class="anticon anticon-check"></i>';
			}
			var gift='<i aria-label="icon: close" class="anticon anticon-close" style="padding-right:40px"></i>';
            if(field['gift']){
			  gift='<i aria-label="icon: close" class="anticon anticon-check" title="'+field['gift']+'" style="padding-right:40px"></i>';
			}
            if (model[1] && ss && takhfif) {
                t = t + '<div class="ant-row-flex ant-row-flex-middle itemList hasgift">' + darsadi + dirkard +
                    '<div class="ant-col-xs-24 ant-col-sm-6 ant-col-md-6 ant-col-lg-6">' +
                    ' <a><img id="LogoImg" width="70" src="' + field['logo'] + '"></a>' +
                    '</div>' +
                    '<div class="ant-col-xs-24 ant-col-sm-6 ant-col-md-6 ant-col-lg-6">' +
                    '<div class="ant-progress ant-progress-line ant-progress-status-active ant-progress-small" style="padding-top:20px">' + star + '</div>' +
                    '</div>' +
                    '<div class="ant-col-xs-24 ant-col-sm-6 ant-col-md-6 ant-col-lg-6">' +
                    '<div class="ant-progress ant-progress-line ant-progress-status-active ant-progress-small" style="padding-top:20px">'+saiar+gift+'</div>' +
                    '</div>' +
                    '<div class="ant-col-xs-24 ant-col-sm-6 ant-col-md-6 ant-col-lg-6">' +
                    '<div style="width:100%;border-style:none;' + padd + ';text-align: left; margin-left: 0; padding-left: 0;margin-bottom:10px">' + off_price + '<span style="font-weight:600;color:#0f2d63;font-size:15px">' + price + ' </span></div>' + btnCashIinstallments + btnAghsat + '</div>' +
                    '<div class="clearfix" style="height:12px;width:100%"></div>' +
                    '<div class="slideUpDow morecome" style="left:30px" id="morecome' + z + '" name="no" onclick="morecom(' + z + ');">اطلاعات بیشتر</div>' +
                    '</div>';
                t = t + '<div class="subBoxList col-md-12 divcom" id="com' + z + '"><div class="ant-row"><div class="ant-col-xs-24 ant-col-sm-12">سهم از بازار : ' + field['market'] + ' درصد</div><div class="ant-col-xs-24 ant-col-sm-12">پرداخت خسارت سیار : <span>' + field['saiar'] + '</span></div><div class="ant-col-xs-24 ant-col-sm-12">رضایت مشتری (رتبه) : ' + field['rezayat'] + '</div><div class="ant-col-xs-24 ant-col-sm-12">تعداد مراکز پرداخت خسارت : ' + field['claim_center_num'] + '</div><div class="ant-col-xs-24 ant-col-sm-12">مدت زمان پاسخگویی به شکایات : ' + field['pasokh'] + ' روز</div><div class="ant-col-xs-24 ant-col-sm-12">سطح توانگری مالی : ' + star + '</div><div class="clearfix ant-col-xs-24 ant-col-sm-12"></div></div></div>';
                z = z + 1;
            } else {
                t = '<div class="ant-row-flex ant-row-flex-middle itemList hasgift" style="background:#fff1f0;border:1px solid #ffa39e">' +
                    '<div class="ant-col-xs-24 ant-col-sm-24 ant-col-md-24 ant-col-lg-24">' +
                    '<div class="ant-progress ant-progress-line ant-progress-status-active ant-progress-small" style="height: 40px; line-height: 50px; text-align:center;">' +
                    'توجه! برای نمایش بسته های پیشنهادی بیمه شخص ثالث ، اطلاعات مورد نیاز ( مشخص شده) را وارد نمایید.</div>' +
                    '</div></div>';
            }
        });
        $('#CompareDiv').html(t);
    });
}

function VehicleType() {
    jQuery("#VehicleType").html("<option value=''>لطفا منتظر بمانید ...</option>")
    jQuery.getJSON("/api/car/CarType", {
        type: 'thirdparty'
    }, function(e) {
        var a = "<option value=''>نوع وسیله نقلیه</option>",
            o = 0;
        jQuery.each(e, function(e, l) {
            if (l.type == Vehicle_Type) {
                a = a + '<option value="' + l.type + '" selected>' + l.title + "</option>";
            } else {
                a = a + '<option value="' + l.type + '">' + l.title + "</option>";
            }
        })
        jQuery("#VehicleType").html(a)
        VehicleBrand()
    })
}

function VehicleBrand() {
    var vehicleType = $('#VehicleType').val()
    jQuery("#vehicleBrand").html("<option value=''>لطفا منتظر بمانید ...</option>")
    jQuery.getJSON("/api/car/CarList", {
        name: vehicleType,
        type: 'thirdparty'
    }, function(e) {
        var a = "<option value=''>برند وسیله نقلیه</option>",
            o = 0;
        jQuery.each(e, function(e, l) {
            if (l.name == machine) {
                a = a + '<option value="' + l.name + '" selected>' + l.name + "</option>";
            } else {
                a = a + '<option value="' + l.name + '">' + l.name + "</option>";
            }
        })
        jQuery("#vehicleBrand").html(a)
    })
}

function VehicleModel() {
    var VehicleModel = $('#vehicleBrand').val()
    jQuery("#vehicleModel").html("<option value=''>لطفا منتظر بمانید ...</option>")
    jQuery.getJSON("/api/car/CarModel", {
        car: VehicleModel
    }, function(e) {
        var a = "<option value=''>مدل وسیله نقلیه</option>",
            o = 0;
        jQuery.each(e, function(e, l) {
            a = a + '<option value="' + l.name + ',' + l.type + ',' + l.id + '">' + l.name + "</option>";
        })
        jQuery("#vehicleModel").html(a)
    })
}
function offcode() {
    $('#codeoff').css('display', 'block').html('<div style="background-image:url(/assets/img/dots_3.gif);height:30px;width:30px;background-size:40px;background-position:center;margin: 0 auto;"></div>')
    var offcode = $('#inputcode').val();
    $.getJSON("/webservice/offcode", {
        code: offcode,
        type: 'thirdparty'
    }, function(e) {
        $.each(e, function(e, l) {
            if (l.status == 200) {
                $('#codeoffhidden').val(l.price + '#' + l.price_type);
                $('#inputcode').prop('disabled', 'disabled')
				$('#codeoff').html('<div style="color:#0c0">کد تخفیف معتبر می باشد</div>');
                change_price();
            } else {
                $('#codeoff').html('<div style="color:#f00">کد تخفیف معتبر نمی باشد</div>');
            }
        })
    })
}
function offs(value,type,i){
 var switcher=$('#switch'+i).attr('name')
 if(type == 'percentage'){
   if(switcher == 'no'){
    offs_percant = parseInt(offs_percant) + parseInt(value);
	$('#switch'+i).attr('name','yes')
   }else{
	offs_percant = parseInt(offs_percant) - parseInt(value); 
	$('#switch'+i).attr('name','no')
   }
 }else{
   if(switcher == 'no'){
    offs_price = parseInt(offs_price) + parseInt(value);
	$('#switch'+i).attr('name','yes')
   }else{
	offs_price = parseInt(offs_price) - parseInt(value);
    $('#switch'+i).attr('name','no')	
   }	
 }
 GetPrice();
}

function OffsThirdparty() {
    $.getJSON("/api/v1/settings/OffsThirdparty", function(e) {
        var a = "";
        $.each(e, function(e, l) {
            a = a + '<div class="ant-col-xs-24 ant-col-sm-24 ant-col-md-24"><div class="ant-row ant-form-item ant-form-no-margin"><div class="ant-form-item-control-wrapper"><div class="ant-form-item-control"><span id="cover_theft_by_break" style="float:left"><input type="checkbox" id="switch'+i_body+'" class="more" name="no" data-off="'+l.value+'" data-type="'+l.type+'" onclick="offs(&apos;'+l.value+'&apos;,&apos;'+l.type+'&apos;,&apos;'+i_body+'&apos;)" style="margin:0;padding:0"/><label class="label" for="switch'+i_body+'">Toggle</label></span><label>'+l.name+'</label></div></div></div></div>';
			i_body++;
        })
        $("#OffsThirdparty").html(a)
    })
}