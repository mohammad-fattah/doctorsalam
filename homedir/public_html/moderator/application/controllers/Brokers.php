...
class Brokers extends MY_Controller {
    ...
    public function index() {
        $this->template->rander("brokers/index");
    }
    ...
    function modal_form() {
        ...
        $this->load->view('brokers/modal_form', $view_data);
    }
	function card_modal_form($user_key) {
        ...
        $this->load->view('brokers/card_modal_form', $view_data);
    }
	...
	function send_to_psp($id) {
        ...
        $view_data['model_info'] = $this->Users_model->get_details($options)->row();
        $this->load->view('brokers/send_to_psp', $view_data);
    }
    ...
    private function _make_row($data, $i) {
        ...
        $delete_link = "";
        $delete_link = anchor(get_uri("brokers/view/" . $data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit'))).js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("brokers/delete/".$data->id), "data-action" => "delete"));
    ...
    function address($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("brokers/address",$view_data);
    }
	function address_form($user_key,$id) {
		...
		$view_data['id']=$id;
        $this->load->view("brokers/address_form",$view_data);
	}

	function discount($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("brokers/discount",$view_data);
    }
	function discount_form($user_key,$id) {
		...
		$view_data['id']=$id;
        $this->load->view("brokers/discount_form",$view_data);
	}
	...
	function wallet($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("brokers/wallet",$view_data);
    }
	function card($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("brokers/card",$view_data);
    }
	...
	private function make_row_discount($data, $i,$user_key) {
        ...
        $delete_link = "";
        $delete_link = modal_anchor(get_uri("brokers/discount_form/".$user_key.'/'.$data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit'))).js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("brokers/delete/".$data->id), "data-action" => "delete"));
        $row_data[] = $delete_link;
        return $row_data;
    }
    private function make_row_address($data, $i,$user_key) {
        ...
        $delete_link = "";
        $delete_link = modal_anchor(get_uri("brokers/address_form/".$user_key.'/'.$data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit'))).js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("brokers/delete/".$data->id), "data-action" => "delete"));
        $row_data[] = $delete_link;
        return $row_data;
    }    
    function delete() {
    ...
    function view($id = 0, $tab = "") {
        if ($id) {
            ...
            if ($user_info) {
                ...
                $this->template->rander("brokers/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function job_info($user_id) {
        ...
        $this->load->view("brokers/job_info", $view_data);
    }
    ...
    function general_info($user_key) {
        ...
        $this->load->view("brokers/general_info", $view_data);
    }
    ...
    function save_general_info($user_id) {
        ...
        //save_custom_fields("broker", $user_id, $this->login_user->is_admin, $this->login_user->user_type);
    ...
    function account_settings($user_key) {
        $view_data['user_info'] = $this->db->get_where("users",array("user_key"=>$user_key))->row();
        $this->load->view("brokers/account_settings", $view_data);
    }
    ...
    function order($user_id) {
        if ($user_id) {
            ...
            $this->load->view("brokers/order", $view_data);
        }
    }
	function all_transaction_client($user_key) {
        $user = $this->db->get_where("users",array("user_key"=>$user_key))->row();
		$key = $user->skype;
        $list_data = $this->db->get_where("orders",array("broker_id"=>$key))->result();
        $result = array();
        $i=1;
        foreach ($list_data as $data) {
	...
	function terminals_info($user_id) {
        if ($user_id) {
            ...
            $this->load->view("brokers/terminals_info", $view_data);
        }
    }
    ...
    function profile($id = 0, $tab = "") {
            ...
            if ($user_info) {
                $view_data['user_info'] = $user_info;
                $this->template->rander("brokers/profile", $view_data);
            } else {
                show_404();
            }
    }
    ...
    function change_password($id = 0) {
        if ($id * 1) {
            ...
            if ($user_info) {
                ...
                $this->template->rander("brokers/change_password", $view_data);
            } else {
                show_404();
            }
        } else {
            //we don't have any specific id to view. show the list of team_member
            $view_data['team_members'] = $this->Users_model->get_details(array("user_type" => "broker", "status" => "active"))->result();
            $this->template->rander("brokers/profile_card", $view_data);
        }
    }
