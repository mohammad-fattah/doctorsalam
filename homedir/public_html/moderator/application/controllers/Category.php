...
class Category extends MY_Controller {
    ...
    function index() {
        ...
        $view_data["categories"] = $this->Category_categories_model->get_details(array("type" => $type, "only_active_categories" => true))->result();
        $view_data["type"] = $type;
    ...
    function view($id = 0) {
        ...
        $view_data['categories'] = $this->Category_categories_model->get_details(array("type" => $model_info->type))->result();
        $view_data['page_type'] = "article_view";

        $view_data['article_info'] = $model_info;

        $this->template->rander('category/articles/view_page', $view_data);
    }
    ...
    function category($id) {
        ...
        $category_info = $this->Category_categories_model->get_one($id);
        ...
        $view_data['categories'] = $this->Category_categories_model->get_details(array("type" => $category_info->type))->result();
	...
	function market($id) {
        ...
        $this->template->rander("category/market/view_page", $view_data);
    }
    ...
    function help_articles() {
        ...
        $view_data["type"] = "help";
        $this->template->rander("category/articles/index", $view_data);
    }
    ...
    function categories() {
        ...
        $view_data["type"] = "help";
        $this->template->rander("category/categories/index", $view_data);
    }
    ...
    function category_modal_form($type) {
        ...
        $view_data['model_info'] = $this->Category_categories_model->get_one($id);
        $view_data['type'] = $type;
        $this->load->view('category/categories/modal_form', $view_data);
    }
    ...
    function save_category() {
        ...
        $save_id = $this->Category_categories_model->save($data, $id);
        if ($save_id) {
    ...
    function categories_list_data($type) {
        ...
        $list_data = $this->Category_categories_model->get_details(array("type" => $type))->result();
        $result = array();
    ...
    private function _category_row_data($id) {
        $options = array("id" => $id);
        $data = $this->Category_categories_model->get_details($options)->row();
    ...
    private function _make_category_row($data) {
        ...
        return array(
            ...
            $data->url,
            $active,
            modal_anchor(get_uri("category/category_modal_form/" . $data->type), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_category'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("category/delete_category"), "data-action" => "delete"))
        );
    }
    ...
    function article_form($type, $id = 0) {
        ...
        $view_data['type'] = $type;
        $view_data['categories_dropdown'] = $this->Category_categories_model->get_dropdown_list(array("title"), "id", array("type" => $type));
        $this->template->rander('category/articles/form', $view_data);
    }
