...
class Clients extends MY_Controller {
	...
	function card_modal_form($user_key) {
        ...
        $this->load->view('clients/card_modal_form', $view_data);
    }
	...
	function comment($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clients/comment",$view_data);
    }
	function comment_list($user_key) {
		$list_data = $this->db->get_where("order_comments",array("user_key"=>$user_key))->result();
		$result = array();
		$i=1;
	...
	private function make_row_comment($data, $i,$user_key) {
		$user = $this->db->get_where('users',array("user_key"=>$data->doctor_id))->row();
	...
	function send_to_psp($id) {
        ...
        $view_data['model_info'] = $this->Users_model->get_details($options)->row();
        $this->load->view('clients/send_to_psp', $view_data);
    }
    ...
    function address($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clients/address",$view_data);
    }
	function address_form($user_key,$id) {
		...
		$view_data['id']=$id;
        $this->load->view("clients/address_form",$view_data);
	}

	function discount($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clients/discount",$view_data);
    }
	function discount_form($user_key,$id) {
		...
		$view_data['id']=$id;
        $this->load->view("clients/discount_form",$view_data);
	}
	...
	function wallet($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clients/wallet",$view_data);
    }
	function card($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clients/card",$view_data);
    }
	...
	private function make_row_discount($data, $i,$user_key) {
        ...
        $delete_link = "";
        $delete_link = modal_anchor(get_uri("clients/discount_form/".$user_key.'/'.$data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit'))).js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("clients/delete/".$data->id), "data-action" => "delete"));
        $row_data[] = $delete_link;
        return $row_data;
    }
    private function make_row_address($data, $i,$user_key) {
        ...
        $delete_link = "";
        $delete_link = modal_anchor(get_uri("clients/address_form/".$user_key.'/'.$data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit'))).js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("clients/delete/".$data->id), "data-action" => "delete"));
        $row_data[] = $delete_link;
        return $row_data;
    }    
    function delete() {
    ...
    function job_info($user_id) {
        ...
        $this->load->view("clients/job_info", $view_data);
    }
    ...
    function general_info($user_key) {
        ...
        $this->load->view("clients/general_info", $view_data);
    }
    ...
    function account_settings($user_key) {
        $view_data['user_info'] = $this->db->get_where("users",array("user_key"=>$user_key))->row();
        $this->load->view("clients/account_settings", $view_data);
    }
    ...
    function order($user_id) {
        if ($user_id) {
            ...
            $this->load->view("clients/order", $view_data);
        }
    }
	...
	function terminals_info($user_id) {
        if ($user_id) {
            ...
            $this->load->view("clients/terminals_info", $view_data);
        }
    }
    ...
    function profile($id = 0, $tab = "") {
            ...
            if ($user_info) {
                $view_data['user_info'] = $user_info;
                $this->template->rander("clients/profile", $view_data);
            } else {
                show_404();
            }
    }
    ...
    function change_password($id = 0) {
        if ($id * 1) {
            ...
            if ($user_info) {
                ...
                $this->template->rander("clients/change_password", $view_data);
            } else {
                show_404();
            }
        } else {
            ...
            $this->template->rander("clients/profile_card", $view_data);
        }
    }
