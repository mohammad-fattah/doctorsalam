...
class Clinic extends MY_Controller {
    ...
    public function index() {
        $this->template->rander("clinic/index");
    }
    ...
    function modal_form() {
        ...
        $this->load->view('clinic/modal_form', $view_data);
    }
	function card_modal_form($user_key) {
        ...
        $this->load->view('clinic/card_modal_form', $view_data);
    }
	...
	function send_to_psp($id) {
        ...
        $view_data['model_info'] = $this->Users_model->get_details($options)->row();
        $this->load->view('clinic/send_to_psp', $view_data);
    }
    ...
    private function _make_row($data, $i) {
        ...
        $delete_link = "";
        $delete_link = anchor(get_uri("clinic/view/" . $data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit'))).js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("clinic/delete/".$data->id), "data-action" => "delete"));
    ...
    function address($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clinic/address",$view_data);
    }
	function times($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clinic/time",$view_data);
    }
	function address_form($user_key,$id) {
		...
		$view_data['id']=$id;
        $this->load->view("clinic/address_form",$view_data);
	}
    function time_form($user_key,$id) {
		$view_data['user_key']=$user_key;
		...
		$view_data['id']=$id;
        $this->load->view("clinic/time_form",$view_data);
	}

	function discount($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clinic/discount",$view_data);
    }
	function discount_form($user_key,$id) {
		...
		$view_data['id']=$id;
        $this->load->view("clinic/discount_form",$view_data);
	}
	...
	function wallet($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clinic/wallet",$view_data);
    }
	function doctor($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clinic/doctor",$view_data);
    }
	function card($user_key) {
		$view_data['user_key'] = $user_key;
        $this->load->view("clinic/card",$view_data);
    }
	...
	private function make_row_discount($data, $i,$user_key) {
        ...
        $delete_link = "";
        $delete_link = modal_anchor(get_uri("clinic/discount_form/".$user_key.'/'.$data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit'))).js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("clinic/delete/".$data->id), "data-action" => "delete"));
        $row_data[] = $delete_link;
        return $row_data;
    }
    private function make_row_address($data, $i,$user_key) {
        ...
        $delete_link = "";
        $delete_link = modal_anchor(get_uri("clinic/address_form/".$user_key.'/'.$data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit'))).js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("clinic/delete/".$data->id), "data-action" => "delete"));
        $row_data[] = $delete_link;
        return $row_data;
    }    
    function delete() {
    ...
    function view($id = 0, $tab = "") {
        if ($id) {
            ...
            if ($user_info) {
                ...
                $this->template->rander("clinic/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function job_info($user_id) {
        ...
        $this->load->view("clinic/job_info", $view_data);
    }
    ...
    function general_info($user_key) {
        ...
        $this->load->view("clinic/general_info", $view_data);
    }
    ...
    function save_general_info($user_id) {
        ...
        save_custom_fields("clinic", $user_id, $this->login_user->is_admin, $this->login_user->user_type);
    ...
    function account_settings($user_key) {
        $view_data['user_info'] = $this->db->get_where("users",array("user_key"=>$user_key))->row();
        $this->load->view("clinic/account_settings", $view_data);
    }
    ...
    function order($user_id) {
        if ($user_id) {
            ...
            $this->load->view("clinic/order", $view_data);
        }
    }
	function all_transaction_client($user_key) {
        $list_data = $this->db->get_where("orders",array("agents_id"=>$user_key))->result();
        $result = array();
        $i=1;
        foreach ($list_data as $data) {
	...
	function terminals_info($user_id) {
        if ($user_id) {
            ...
            $this->load->view("clinic/terminals_info", $view_data);
        }
    }
    ...
    function profile($id = 0, $tab = "") {
            ...
            if ($user_info) {
                $view_data['user_info'] = $user_info;
                $this->template->rander("clinic/profile", $view_data);
            } else {
                show_404();
            }
    }
    ...
    function change_password($id = 0) {
        if ($id * 1) {
            ...
            if ($user_info) {
                ...
                $this->template->rander("clinic/change_password", $view_data);
            } else {
                show_404();
            }
        } else {
            //we don't have any specific id to view. show the list of team_member
            $view_data['team_members'] = $this->Users_model->get_details(array("user_type" => "clinic", "status" => "active"))->result();
            $this->template->rander("clinic/profile_card", $view_data);
        }
    }
