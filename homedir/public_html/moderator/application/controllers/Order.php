...
class Order extends MY_Controller {
	...
	function client($user_id) {
        $this->template->rander("order/client");	
    }
    ...
    private function _make_category_row($data) {
		...
		return array(
			...
			modal_anchor(get_uri("order/edit_status_modal_form/" . $data->id."/".$data->status), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => "جزییات"))
        );
    }
	...
	function edit_status_modal_form($id , $current_status){
		...
		$this->load->view('order/client/edit_status_modal_form' , $view_data);
	}
	...
	private function _make_transaction_row_broker($data,$i) {
        ...
        return array(
            ...
            $data->CashIinstallments,
            $data->status,
            anchor(get_uri("order/view/" . $data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => "جزییات"))
        );
    }
	function delete() {
        validate_submitted_data(array(
	...
	function document_info($order_id) {
        if ($order_id) {
            ...
            $this->load->view("order/document_info", $view_data);
        }
    }
	function sell_info($order_id) {
        if ($order_id) {
			$view_data['id'] = $order_id;
            $this->load->view("order/sell_info", $view_data);
        }
    }
	function insure_info($order_id) {
        if ($order_id) {
			$view_data['id'] = $order_id;
            $this->load->view("order/insure_info", $view_data);
        }
    }
	function status_info($order_id) {
	 $view_data['id'] = $order_id;
     $this->load->view("order/status_info", $view_data);
    }
	...
	function general_info($order_id) {
        //if ($order_id) {
			...
			$this->load->view("order/general_info", $view_data);
        //}
    }
  ...
  curl_close($curl);
    ...
    function action()
    {
        ...
        foreach($reminder_data as $row)
        {
            ...
            $object->getActiveSheet()->setCellValueByColumnAndRow(0, $excel_row, $row->start_date);
