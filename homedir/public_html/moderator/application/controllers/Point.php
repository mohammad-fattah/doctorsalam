...
class Point extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->load->model("Point_categories_model");
    ...
    function index() {
        ...
        $view_data["categories"] = $this->Point_categories_model->get_details(array("type" => $type, "only_active_categories" => true))->result();
        $view_data["type"] = $type;
        $this->template->rander("point/index", $view_data);
    }
    function client() {
        $view_data["extant"] = '';
        $this->template->rander("point/point_client", $view_data);
    }
	function all_transaction($user_id) {
        ////$this->access_only_allowed_members();

        $list_data = $this->Point_categories_model->all_transaction($user_id)->result();
        $result = array();
	...
	function _make_transaction_row($data) {
		...
		if($data->status=='active'){$status='<span style="color:#0ea20e;font-weight:bold">تایید شده</span>';}else{$status='<span style="color:#ff0e00;font-weight:bold">تایید نشده</span>';}
        return array(
            $data->id,
            $data->for,
            $data->date,
            number_format($data->amount).' امتیاز',
            number_format($data->extant).' امتیاز',
            $type,
			$status
        );
    }
    ...
    function view($id = 0) {
        ...
        $view_data['categories'] = $this->Point_categories_model->get_details(array("type" => $model_info->type))->result();
        $view_data['page_type'] = "article_view";

        $view_data['article_info'] = $model_info;

        $this->template->rander('point/articles/view_page', $view_data);
    }
    ...
    function category($id) {
        ...
        $category_info = $this->Point_categories_model->get_one($id);
        ...
        $view_data['categories'] = $this->Point_categories_model->get_details(array("type" => $category_info->type))->result();
	...
	function market($id) {
        ...
        $this->template->rander("point/market/view_page", $view_data);
    }
    ...
    function help_articles() {
        ...
        $view_data["type"] = "help";
        $this->template->rander("point/articles/index", $view_data);
    }
    ...
    function categories() {
        ...
        $view_data["type"] = "help";
        $this->template->rander("point/categories/index", $view_data);
    }
    ...
    function category_modal_form($type) {
        ...
        $view_data['model_info'] = $this->Point_categories_model->get_one($id);
        $view_data['type'] = $type;
        $this->load->view('point/categories/modal_form', $view_data);
    }
    ...
    function save_category() {
        ...
        $save_id = $this->Point_categories_model->save($data, $id);
        if ($save_id) {
    ...
    function categories_list_data($type) {
        ...
        $list_data = $this->Point_categories_model->get_details(array("type" => $type))->result();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _category_row_data($id) {
        $options = array("id" => $id);
        $data = $this->Point_categories_model->get_details($options)->row();
    ...
    function article_form($type, $id = 0) {
        ...
        $view_data['type'] = $type;
        $view_data['categories_dropdown'] = $this->Point_categories_model->get_dropdown_list(array("title"), "id", array("type" => $type));
        $this->template->rander('point/articles/form', $view_data);
    }
