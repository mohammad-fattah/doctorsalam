...
class Projects extends MY_Controller {
    ...
    function save() {
        ...
        if ($save_id) {
            ...
            save_custom_fields("orders", $save_id, $this->login_user->is_admin, $this->login_user->user_type);
	...
	function cash_info_merchant($team_member_id = 0) {
        //$this->access_only_team_members();
        ...
        $options["merchant_id"] = $team_member_id;
        $options["peyment"] = 'نقد';


        $list_data = $this->Projects_model->get_cash($options)->result();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_row($data, $custom_fields);
        }
	...
	function cash_info_client($team_member_id = 0) {
        //$this->access_only_team_members();
        ...
        $options["client_id"] = $team_member_id;
        $options["peyment"] = 'نقد';


        $list_data = $this->Projects_model->get_cash_client($options)->result();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_row($data, $custom_fields);
        }
	...
	function credit_info_merchant($team_member_id = 0) {
        //$this->access_only_team_members();
        ...
        $options["merchant_id"] = $team_member_id;
        $options["peyment"] = 'اقساط';


        $list_data = $this->Projects_model->get_credit($options)->result();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_row($data, $custom_fields);
        }
    ...
    function projects_list_data_of_client($client_id,$dd) {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $optoins = "";
        if ($this->can_edit_projects()) {
            $optoins.=modal_anchor(get_uri("projects/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit'), "data-post-id" => $data->id));
        }
        ...
        $row_data = array(
            ...
            anchor(get_uri("clients/view/" . $data->client_id), $data->first_name.' '.$data->last_name),
            $price,
            $data->start_date,
            $start_date,
    ...
    private function _get_project_info_data($project_id) {
        ...
        if ($project_info) {
            ...
            //$view_data['project_progress']=100;
            return $view_data;
        } else {
            show_404();
        }
    ...
    function show_my_starred_projects() {
        $view_data["orders"] = $this->Projects_model->get_starred_projects($this->login_user->id)->result();
    ...
    function overview($project_id) {
        ...
        $view_data['custom_fields_list'] = $this->Custom_fields_model->get_combined_details("orders", $project_id, $this->login_user->is_admin, $this->login_user->user_type)->result();
    ...
    private function _make_file_row($data) {
        ...
        $description = "<div class='pull-left'>" .
                js_anchor($data->description, array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "1", "data-url" => get_uri("projects/view_file/" . $data->id)));
