...
class Settings extends MY_Controller {
    ...
    function save_general_settings() {
        $settings = array("site_logo","app_title", "language", "date_format", "time_format", "rows_per_page","message_thirdparty","message_body","message_life","message_fire","message_travel","message_off","message_health","longitude","latitude","site_background","app_background","company_address","company_phone","company_email","company_website","company_facebook","company_aparat","company_twitter","company_telegram","company_instagram","ios_app_link","android_app_link");
        ...
        foreach ($settings as $setting) {
			...
			if ($setting === "site_logo") {
				$this->Settings_model->save_setting($setting, '/moderator/files/system/'.$value);
			}else{
				$this->Settings_model->save_setting($setting, $value);
			}
        }
    ...
    function save_profile_image() {
        ...
        if ($profile_image) {
			...
			$this->db->where(array('setting_name'=>'agent_pic'));
            $this->db->update("settings",array("setting_value" => $image_data));
    ...
    function save_company_settings() {
        $settings = array("company_address", "company_phone", "company_email", "company_website","company_fax","company_telegram","company_twitter","company_facebook","company_instagram","company_email_sell");
