...
class Team_members extends MY_Controller {
    ...
    function add_team_member() {
        ...
        if ($this->Users_model->is_email_exists($this->input->post('email'),'staff')) {
            echo json_encode(array("success" => false, 'message' => "ایمیل از قبل ثبت شده است"));
            exit();
        }
		if ($this->Users_model->is_melli_code_exists($this->input->post('melli_code'),'staff')) {
            echo json_encode(array("success" => false, 'message' => "کد ملی از قبل ثبت شده است"));
            exit();
        }
		if ($this->Users_model->is_phone_exists($this->input->post('phone'),'staff')) {
