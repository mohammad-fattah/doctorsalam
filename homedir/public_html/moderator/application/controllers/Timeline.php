...
class Timeline extends MY_Controller {
    ...
    function index() {       
        $members_options = array(
            "status" => "active",
            "user_type" => "staff",
            "chat_unique_id" => $this->input->post("team_member")
        );
    ...
    /* save a post */
        ...
        {
            $id = $_POST['unique_id'];
            $sub_kriterias = $this->Posts_model->get_details($id);
            $result = array();
            $i=1;
            foreach ($sub_kriterias as $data) {                
                $result[] = $this->_make_row($data,$i,$id);
                $i++;
            }
            echo json_encode($result, JSON_UNESCAPED_UNICODE);
            // exit();
        }
    private function _make_row($data, $i, $id) {
...
// ******************************************save reply    
        ...
        if ($this->input->post('source_description')) {
        ...
        $data = array(
            ...
            "created_for" => $this->input->post('unique_id'),                     
