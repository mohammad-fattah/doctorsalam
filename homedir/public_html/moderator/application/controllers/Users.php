...
class Users extends MY_Controller {
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->Users_model->get_one($client_id);
    ...
    function save() {
        ...
        $save_id = $this->Users_model->save($data, $client_id);
        if ($save_id) {
    ...
    function list_data() {
        ...
        $list_data = $this->Users_model->get_details($options,'client')->result();
        $result = array();
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $row_data = array(
		    $data->id,
			anchor(get_uri("users/view/" . $data->id), $data->first_name.' '.$data->last_name),
			$data->phone,
			$data->melli_code,
            //to_decimal_format($data->total_projects),
            to_currency($data->invoice_value, $data->currency_symbol).' / '.to_currency($data->invoice_value, $data->currency_symbol),
            to_decimal_format($data->total_projects).' / '.to_decimal_format($data->total_projects),
			to_decimal_format($data->total_projects),
        );
        ...
        $row_data[] = modal_anchor(get_uri("cards/category_modal_form/"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_client'), "data-post-id" => $data->id))
                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_client'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("users/delete"), "data-action" => "delete"));
    ...
    function view($client_id = 0, $tab="") {
        ...
        if ($client_id) {
            ...
            $client_info = $this->Users_model->get_details($options)->row();
            if ($client_info) {
                ...
                $this->template->rander("users/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function add_remove_star($client_id, $type = "add") {
        if ($client_id) {
            ...
            if ($type === "add") {
                $this->Users_model->add_remove_star($client_id, $this->login_user->id, $type = "add");
                $this->load->view('users/star/starred', $view_data);
            } else {
                $this->Users_model->add_remove_star($client_id, $this->login_user->id, $type = "remove");
                $this->load->view('users/star/not_starred', $view_data);
            }
        }
    }

    function show_my_starred_clients() {
        $view_data["clients"] = $this->Users_model->get_starred_clients($this->login_user->id)->result();
        $this->load->view('users/star/clients_list', $view_data);
    }
    ...
    function projects($client_id) {
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view("users/projects/index", $view_data);
    }
	function cash($client_id) {
        $this->access_only_allowed_members();
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view("users/cash/index", $view_data);
    }
	function credit($client_id) {
        $this->access_only_allowed_members();
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view("users/credit/index", $view_data);
    }
    ...
    function payments($client_id) {
        ...
        if ($client_id) {
            $view_data["client_info"] = $this->Users_model->get_one($client_id);
            $view_data['client_id'] = $client_id;
            $this->load->view("users/payments/index", $view_data);
        }
    }
    ...
    function tickets($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("users/tickets/index", $view_data);
        }
    }
    ...
    function invoices($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("users/invoices/index", $view_data);
        }
    }
    ...
    function estimates($client_id) {
        ...
        if ($client_id) {
            $view_data["client_info"] = $this->Users_model->get_one($client_id);
            $view_data['client_id'] = $client_id;
            $this->load->view("users/estimates/estimates", $view_data);
        }
    }
    ...
    function estimate_requests($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("users/estimates/estimate_requests", $view_data);
        }
    }
    ...
    function notes($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("users/notes/index", $view_data);
        }
    }
    ...
    function contact_profile($contact_id = 0, $tab = "") {
        ...
        $view_data['client_info'] = $this->Users_model->get_one($view_data['user_info']->client_id);
        $view_data['tab'] = $tab;
        if ($view_data['user_info']->user_type === "client") {
            ...
            $this->template->rander("users/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function contacts($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("users/contacts/index", $view_data);
        }
    }
    ...
    function add_new_contact_modal_form() {
        ...
        $this->load->view('users/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            $this->load->view('users/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    function company_info_tab($client_id = 0) {
        if ($client_id) {
            ...
            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $this->load->view('users/contacts/company_info_tab', $view_data);
        }
    }
    ...
    function save_contact() {
        ...
        $primary_contact = $this->Users_model->get_primary_contact($client_id);
        if (!$primary_contact) {
    ...
    private function _make_contact_row($data, $custom_fields) {
        ...
        $contact_link = anchor(get_uri("users/contact_profile/" . $data->id), $full_name . $primary_contact);
        ...
        $row_data[] = js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("users/delete_contact"), "data-action" => "delete"));
    ...
    function invitation_modal() {
        ...
        $view_data["client_info"] = $this->Users_model->get_one($client_id);
        $this->load->view('users/contacts/invitation_modal', $view_data);
    }
    ...
    function users() {
        if ($this->login_user->user_type === "client") {
            ...
            $this->template->rander("users/contacts/users", $view_data);
        }
    }
