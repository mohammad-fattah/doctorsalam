...
class Viewpoint extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("viewpoint/index", $view_data);
    }
    ...
    function form($id = 0) {
        ...
        $view_data['model_info'] = $this->Viewpoint_modal->get_one($id);
        $view_data['share_with'] = $id ? explode(",", $view_data['model_info']->share_with) : array("all_members");
        $this->template->rander('viewpoint/modal_form', $view_data);
    }
    ...
    function view($id = "") {
        if ($id) {
            ...
            $announcement = $this->Viewpoint_modal->get_details($options)->row();
            if ($announcement) {
                $view_data['announcement'] = $announcement;

                //mark the announcement as read for loged in user
                $this->Viewpoint_modal->mark_as_read($id, $this->login_user->id);
                return $this->template->rander("viewpoint/view", $view_data);
            }
        }
    ...
    function delete() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Viewpoint_modal->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, lang('error_occurred')));
            }
        } else {
            if ($this->Viewpoint_modal->delete($id)) {
    ...
    function list_data() {
        ...
        $list_data = $this->Viewpoint_modal->get_details($options)->result();
        $result = array();
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Viewpoint_modal->get_details($options)->row();
    ...
    private function _make_row($data) {
        ...
        $option = "";
        if ($this->access_type === "all") {
            $option = anchor(get_uri("viewpoint/form/".$data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_announcement')))
                    . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_announcement'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("viewpoint/delete"), "data-action" => "delete"));
        }
        return array(
            anchor(get_uri("viewpoint/view/" . $data->id), $data->title, array("class" => "", "title" => lang('view'))),
