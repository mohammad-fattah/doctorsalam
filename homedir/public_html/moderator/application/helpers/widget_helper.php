 ...
 * @return html
 */
if (!function_exists('client_dashboard_widget')) {

    function client_dashboard_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["clock_status"] = $ci->Sell_model->count_dashboard_client($ci->login_user->id);
        $ci->load->view("sell/client_dashboard_widget", $view_data, $return_as_data);
    }
...
if (!function_exists('open_tickets_widget')) {
    ...
    function open_tickets_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["total"] = $ci->Posts_model->count_new_posts();
        $ci->load->view("timeline/open_tickets_widget", $view_data, $return_as_data);
    }
...
if (!function_exists('project_status_widget')) {

    function project_status_widget($user_id = 0, $return_as_data = false) {
        $ci = get_instance();
        $options = array(
            "user_id" => $user_id ? $user_id : $ci->login_user->id
        );
        ...
        $view_data["project_open"] = $info->open;
        $view_data["project_completed"] = $info->completed;
        $ci->load->view("projects/events_today", $view_data, $return_as_data);
    }

}
if (!function_exists('money_widget')) {

    function money_widget($user_id = 0, $return_as_data = false) {
        $ci = get_instance();
        $options = array(
            "user_id" => $user_id ? $user_id : $ci->login_user->id
        );
        ...
        $view_data["project_open"] = $info->open;
        $view_data["project_completed"] = $info->completed;
        $ci->load->view("projects/money_widget", $view_data, $return_as_data);
    }

}
if (!function_exists('aghsat_widget')) {

    function aghsat_widget($user_id = 0, $return_as_data = false) {
        $ci = get_instance();
        $options = array(
            "user_id" => $user_id ? $user_id : $ci->login_user->id
        );
        ...
        $view_data["project_open"] = $info->open;
        $view_data["project_completed"] = $info->completed;
        $ci->load->view("projects/aghsat_widget", $view_data, $return_as_data);
    }
