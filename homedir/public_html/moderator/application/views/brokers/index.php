            ...
            <div class="title-button-group">
                <?php
				 if($this->login_user->user_type !== 'broker' || $this->login_user->user_type !== 'agent')
                  echo modal_anchor(get_uri("brokers/modal_form"), "<i class='fa fa-plus-circle'></i> افزودن بازاریاب", array("class" => "btn btn-default", "title" => "افزودن بازاریاب"));
                ?>
            </div>
        </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#team_member-table").appTable({
            source: '<?php echo_uri("brokers/list_data/") ?>',
            order: [[1, "asc"]],
            columns: [
                {title: "ردیف", "class": "text-center w50"},
                {title: "نام بازاریاب", "class": "text-center w100"},
                ...
                {title: "آدرس مجازی", "class": "text-center w50"},
