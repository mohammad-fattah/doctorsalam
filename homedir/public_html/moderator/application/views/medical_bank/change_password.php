...
<div id="page-content" class="p220 clearfix">
    <div class="page-title clearfix no-border bg-off-white" style="margin-bottom:10px">
      <h1>
        جزئیات کاربر - <?php echo $user_info->first_name.' '.$user_info->last_name; ?>  
      </h1>
    </div>
    ...
    <ul data-toggle="ajax-tab" class="nav nav-tabs" role="tablist" style="display:none">
        ...
        <?php //if ($show_general_info) { ?>
            <!--<li><a  role="presentation" href="<?php echo_uri("doctor/general_info/" . $user_info->id); ?>" data-target="#tab-general-info">مشخصات کاربر</a></li>-->
        <?php //} ?>
        <?php if ($show_account_settings) { ?>
            <li><a role="presentation" href="<?php echo_uri("doctor/account_settings/" . $user_info->id); ?>" data-target="#tab-account-settings"> <?php echo lang('account_settings'); ?></a></li>
        <?php } ?>

        <?php if ($show_projects) { ?>
            <li><a role="presentation" href="<?php echo_uri("doctor/cash_info/" . $user_info->id); ?>" data-target="#tab-cash-info">تراکنش های نقدی</a></li>
			<li><a role="presentation" href="<?php echo_uri("doctor/credit_info/" . $user_info->id); ?>" data-target="#tab-credit-info">تراکنش های اقساطی</a></li>
        <?php } ?>
    </ul>
