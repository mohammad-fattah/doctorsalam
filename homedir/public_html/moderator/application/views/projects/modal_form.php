...
<div class="modal-body clearfix">
    ...
    <div class="form-group">
        <label for="project_labels" class=" col-md-3">استان</label>
        <div class=" col-md-9">
            <?php
            echo form_input(array(
                "id" => "project_city",
                ...
                "class" => "form-control",
                "placeholder" => "استان درخواست داده شده"
            ));
            ?>
        </div>
    </div>
	...
	<div class="form-group">
        <label for="project_labels" class=" col-md-3">ارجاع به</label>
        <div class=" col-md-9">
		   <?php if ($model_info->agent): ?>
            <?php
			 echo form_dropdown("client_id", $clients_dropdown, array($model_info->client_id), "class='select2 validate-hidden' value='".$model_info->agent."' data-rule-required='true', data-msg-required='" . lang('field_required') . "'");
            ?>
		   <?php else: ?>
			<?php
                //echo form_dropdown("client_id", $clients_dropdown, array($model_info->client_id), "class='select2 validate-hidden' value='".$model_info->agent."' data-rule-required='true', data-msg-required='" . lang('field_required') . "'");
    ...
    <?php if ($model_info->id) { ?>
        <div class="form-group">
            ...
            <div class=" col-md-9">
                <?php
                echo form_dropdown("status", array("open" => lang("open"), "erja" => "ارجاع داده شده", "sodor" => "در حال صدور", "sader" => "صادره شده", "tahvil" => "تحویل داده شده","completed" => lang("completed"), "canceled" => lang("canceled")), array($model_info->status), "class='select2'");
                ?>
            </div>
        </div>
    <?php } ?>
